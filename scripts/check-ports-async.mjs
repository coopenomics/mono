 ...
 *     разбираются заранее (487-19), а не на переезде.
 *
...
/** Синхронные методы портов, которые границу пересекут. Долг 487-19. */
const KNOWN_SYNC = {
  ...
  'IChainChangesPort.declareTables': 'реестр таблиц ленты изменений, передаются данные',
  'IChainChangesPort.declareLocalTables': 'реестр таблиц ленты изменений, передаются данные',
  'IChainChangesPort.setStaff': 'реестр персонала расширения в ленте изменений, передаётся список',
...
/** Сигнатуры, передающие объект с методами. Настоящий предмет 487-19. */
...
console.log(
  `  порты выносимых расширений (${PORTABLE.join(', ')}): ` +
    `${syncFound.length} синхронных, ${handleFound.length} с объектом-обработчиком — известные, долг 487-19`
);
