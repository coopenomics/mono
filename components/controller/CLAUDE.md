...
`forwardRef` не добавляется «чтобы заработало». Обёртка, поставленная наугад,
выглядит точно так же, как необходимая, и отличить их чтением нельзя — так
накопилось полсотни штук, из которых реальный цикл обходили семь.

Прежде чем ставить обёртку:

```bash
node scripts/analyze-cycles.mjs        # какие обёртки обходят реальный цикл, какие лишние
```
...
к владельцу. Нет пути — обёртка лишняя, снимать.

Если цикл настоящий: **каждая оставшаяся обёртка сопровождается комментарием**
— какой именно цикл она обходит (путь модулей) и почему тот законен. Канон:
`application/gateway/gateway.module.ts`, `domain/registration/registration-domain.module.ts`,
`extensions/capital/application/syncers/commit-sync.service.ts`.

Обычный порядок действий, когда цикл настоящий, но не обязан быть: вынести
общий кусок в третий модуль либо инвертировать зависимость портом
`@coopenomics/innercoop`. «Оставить как есть» — тоже решение, но записанное.
