...
- **Метод порта возвращает `Promise` (ADR-18).** Порт обязан пережить вынос расширения за границу процесса: сегодня адаптер зовёт сервис ядра напрямую, завтра уходит по сети, и подменяется только он. Синхронную сигнатуру так не подменить — переписывать придётся каждого потребителя. Гейт `scripts/check-ports-async.mjs` (в `pnpm check`) держит список известных исключений (`KNOWN_SYNC`, 23 метода — долг FC1-22); новый синхронный метод в нём отсутствует и роняет проверку. По той же причине **порт не передаёт объект с методами**: по сети его не отдать. Реестры, которые так делают (права стола, провайдеры платежей, хуки оферт), — часть того же долга.
