...
/**
 * Приём в пайщики выпускает свидетельство по связи, ждавшей решения совета (story 7.5).
 *
 ...
 * связь ждёт, а выпуск происходит по записи цепи о приёме.
 */
...
describe('Приём пайщика в кооператив', () => {
  ...
  it('сбой выпуска не роняет обработку события: связь не потеряна, она ждёт', async () => {
    const { service } = build(
      jest.fn(async () => {
