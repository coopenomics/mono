...
/**
 * Приём уведомлений о связях (story 7.2).
 *
...
describe('Уведомление о связи карты с кооперативом', () => {
  ...
  it('кандидат связал карту до приёма — связь ждёт решения совета, а не теряется', async () => {
    // Свидетельствовать нечего: в реестре пайщиков цепи его нет. Но человек пришёл со своей картой
    // ровно затем, чтобы не заводить вторую, — потерять связь значит обнулить накопленное
    // (story 7.5).
    const withNumber = { ...notification, card_number: '9689205327798678' };
