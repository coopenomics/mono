 ...
 * показать, — он сам выдавал свидетельство и сам принимал уведомление о связи.
 */
...
describe('Карта кооператора в столе кооператива', () => {
  ...
  it('свидетельство ещё не доехало — карта всё равно показана как выпущенная', async () => {
    // Запись журнала появляется по уведомлению о связи: карта у человека уже есть, даже
  ...
  it('карта связана при вступлении, свидетельства ещё нет — стол показывает её выпущенной', async () => {
    // Пайщик принят, а свидетельство по связи ещё не доставлено в сеть. Сказать
    // человеку «карта не выпущена» было бы неправдой: он её выпустил и связал
    // (story 7.5). Кандидату до приёма стол закрыт (решение владельца 25.09.2026).
