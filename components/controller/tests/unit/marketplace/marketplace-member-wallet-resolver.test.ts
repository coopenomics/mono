 ...
 *   - `w.wal.member` — универсальный членский (program_id=1).
 *   - `w.mkt.member` — программный членский Стола Заказов (program_id=2).
 *   - `w.mkt.order`  — резерв под Order (program_id=2). Введён 2026-05-28
 *                       взамен механики .blocked на w.mkt.member.
 *
 ...
 * `w.mkt.member` появляется только после первого `orderoffer`/`createorder`,
 * а `w.mkt.order` — только после первого `createorder`.
 */
...
describe('MarketplaceMemberWalletResolver', () => {
  it('пайщик с записями share + mkt.member + mkt.order → массив из 4 кошельков', async () => {
    const repo = makeRepo([
  ...
  it('human_name подтягивается из cooptypes LEDGER2_WALLET_REGISTRY', async () => {
    ...
    expect(dto.wallets[3].human_name).toBe('ЦПП «Стол Заказов» — резерв под заказ у пайщика');
  });
