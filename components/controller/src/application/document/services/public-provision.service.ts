...
const PUBLIC_REGISTRY_IDS: readonly number[] = [
  ...
  // Пользовательское соглашение подставляет имя подписанта, но публично
  // показывается бланком: без username фабрика оставляет на его месте прочерк.
  Cooperative.Registry.UserAgreement.registry_id,
];

/**
 * Сколько держим собранный текст, прежде чем снова идти в цепь. Час выбран из
 * расчёта нагрузки, а не свежести: положение меняется решением совета — редко и
 * планово, — а страницу открывают постоянно, и каждый заход иначе стоил бы двух
 * запросов в цепь. Задержка в пределах часа после смены редакции безвредна.
 */
...
export class PublicProvisionService {
   ...
   * Имя подписанта в публичном показе не подставляется — там, где документ его
   * ждёт, остаётся прочерк.
