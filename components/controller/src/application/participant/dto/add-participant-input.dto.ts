...
export class AddParticipantInputDTO {
  ...
  @IsNotEmpty({ message: validationMessage('participation.addParticipantInput.fieldCreatedAtRequired') })
  created_at!: string;

  @Field({ description: 'Электронная почта' })
  @IsNotEmpty({ message: validationMessage('participation.addParticipantInput.fieldEmailRequired') })
  email!: string;
  ...
  @IsNotEmpty({ message: validationMessage('participation.addParticipantInput.fieldTypeRequired') })
  type!: AccountType;

  @Field({ description: 'Вступительный взнос, который был внесён пайщиком' })
  @IsNotEmpty({ message: validationMessage('participation.addParticipantInput.fieldInitialRequired') })
  initial!: string;

  @Field({ description: 'Минимальный паевый взнос, который был внесён пайщиком' })
  @IsNotEmpty({ message: validationMessage('participation.addParticipantInput.fieldMinimumRequired') })
  minimum!: string;

  @Field({ description: 'Флаг распределения вступительного взноса в невозвратный фонд вступительных взносов кооператива' })
  @IsNotEmpty({ message: validationMessage('participation.addParticipantInput.fieldSpreadInitialRequired') })
  ...
  @IsNotEmpty({
    message: validationMessage('participation.addParticipantInput.atLeastOneDataRequired'),
  })
