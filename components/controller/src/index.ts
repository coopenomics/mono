...
async function bootstrap() {
  ...
  // Непринятые миграции — до подъёма Nest, чтобы схема была готова к первому
  // запросу, а не появлялась под уже принимающим трафик приложением.
  //
  ...
  // сразу и по частям, и причину потом ищут долго. Лучше упасть здесь, где
  // видно, что именно не прошло.
  if (config.auto_migrate) {
    logger.info('AUTO_MIGRATE включён — прогоняем непринятые миграции перед стартом');
    try {
      await migrateData();
    } catch (error) {
