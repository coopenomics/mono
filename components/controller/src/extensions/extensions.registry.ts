...
export const AppRegistry: INamedExtension = {
  ...
  support: {
    ...
    is_internal: true,
    availability: ExtensionAvailability.EVERYWHERE,
    // Решение председателя: имя `support` закреплено за этим столом. Занимавший
    // его пункт «Поддержка» с маршрутом `/:coopname/support` убран из
    // расширения `participant` тем же диффом, что заведены эти столы: два
    // маршрута с одним именем Vue Router либо молча перезатирает, либо роняет.
    // Кнопка вызова чата Chatwoot переехала на стол пайщика.
    //
    // Столов два, а не один: у них разный состав операций — очередь и счётчики
    // доступны только совету, а стол пайщика их не знает вовсе. Видимость
    // решается правами (`SupportDesktopGrantsProvider`), а не ролью в маршруте.
    desktops: [
      {
        name: 'support',
        title: 'Поддержка',
