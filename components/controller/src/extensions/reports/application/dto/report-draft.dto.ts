...
export class SaveReportDraftInputDTO {
  ...
  // Верхняя граница — общий предохранитель на все формы, а не срок конкретной:
  // у квартальных период 1–4, у месячных 1–12, у уведомления по НДФЛ 1–24
  // (по два расчётных периода в месяце). Допустимость периода для своего типа
  // отчёта проверяют edits-DTO, здесь черновик просто не должен принять мусор.
