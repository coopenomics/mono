...
export class Ndfl6CertificateEditsInputDTO {
  ...
  @Matches(/^\d{2}$/, { message: t('reports.ndfl6Edits.correctionNumberFormat') })
  ...
  @Matches(DATE_DDMMYYYY_PATTERN, { message: t('reports.ndfl6Edits.birthDateFormat') })
  birthDate!: string;

  @Field(() => String, { description: 'Код статуса налогоплательщика: 1 — резидент РФ' })
  @Matches(/^\d$/, { message: t('reports.ndfl6Edits.taxpayerStatusFormat') })
  taxpayerStatus!: string;

  @Field(() => String, { description: 'Гражданство, код страны по ОКСМ: 643 — Россия' })
  @Matches(/^\d{3}$/, { message: t('reports.ndfl6Edits.citizenshipCodeFormat') })
  citizenshipCode!: string;

  @Field(() => String, {
    description: 'Код вида документа, удостоверяющего личность: 21 — паспорт РФ',
  })
  @Matches(/^\d{2}$/, { message: t('reports.ndfl6Edits.documentTypeCodeFormat') })
