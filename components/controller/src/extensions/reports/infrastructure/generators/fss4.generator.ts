...
export class Fss4Generator implements IReportGenerator {
  ...
  private buildXml(edits: ZeroReportEditsShape): string {
    ...
    // (XXX-XXX-XXXXXX), а не единый 10-значный номер СФР — так требует
    // СберКорус («Сфера Отчётность»), через который кооператив сдаёт отчёт:
    // с номером СФР он отчёт не принимает, а при собственной генерации
    // подставляет в этот тег номер ПФР.
    //
    // Выбор номера зависит от оператора, а не от XSD: Контур-Экстерн,
    // например, кладёт сюда 10-значный номер СФР. При переходе на другого
    // оператора отчётности это место, скорее всего, придётся править — либо
    // сделать выбор номера настройкой (в одном месте ПФР, в другом СФР),
    // либо переключить на sfrRegNumber. Оба номера есть в реквизитах
    // кооператива, так что менять надо только подстановку здесь и в имени
