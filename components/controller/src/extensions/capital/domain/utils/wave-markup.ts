 ...
 *
 * Таймфрейм один — день: ряд приходит дневными барами, «период» ниже
 * везде означает сутки.
 *
...
/** Окно свежей активности: неделя дневных баров */
export const ACTIVITY_LOOKBACK_DAYS = 7;

/**
 * Энергия движения за последние lookback дней → [0..1].
 * 0 — ряд стоит (LEVEL: уровень не меняется / RATE: Δ≈0);
 * 1 — есть заметная активность (полный волновой разгон).
 *
 * Окно — неделя, а не пара дней: на дневках работа идёт неровно
 * (выходные, день без задач), и по 2–4 барам «стоим на месте» ловилось бы
 * на каждом перерыве, схлопывая коридор в плоскую линию.
 */
export function recentActivityScore(
  values: number[],
  mode: MetricSeriesMode,
  lookback = ACTIVITY_LOOKBACK_DAYS
): number {
