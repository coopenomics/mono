...
export class ProjectPermissionsOutputDTO {
  ...
  /**
   * Может ли пользователь устанавливать приоритет проекта/компонента.
   * Проект: только председатель. Компонент: председатель или мастер родительского проекта.
   */
  @Field(() => Boolean, {
    description: 'Может ли устанавливать приоритет проекта или компонента',
  })
