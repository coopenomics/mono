...
export class MarketplaceAplReceptionService {
  ...
  async create(
    ...
    // поставщика уведомляем АСИНХРОННО (Novu), экран ему не перекрываем.
    ...
    // гейт «Подпишите документ», и он подписал, не уходя. Это РАЗНЫЕ каналы:
    // Б → Novu-уведомление, А → ws-сигнал realtime-каналу для overlay.
