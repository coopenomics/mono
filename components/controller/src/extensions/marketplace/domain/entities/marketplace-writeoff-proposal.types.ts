...
/**
 * Позиция к списанию. Опциональный `inventory_id` — пометка, какой
 * штрих-код из `marketplace_inventory` попал в корзину (нужно крону для
 * последующего пометить статус позиции на WRITTEN_OFF после executed).
 */
