...
export class CardcoopMembershipService implements OnModuleDestroy {
   ...
   *
   * Идемпотентно по паре «пайщик и карта»: повторное уведомление о той же связи
   ...
   * @param cardId — карта держателя из уведомления о связи.
   ...
   *
   * У записей, заведённых до story 7.4, номера нет, а повторное уведомление о связи —
   ...
   * на которую документ обязан опираться. Но и потерять связь нельзя — человек пришёл со
  ...
  /**
   * Выпускает подтверждение по связи, ждавшей приёма (story 7.5).
   *
   ...
   *   его уведомление о связи мы уже подтвердили ответом 200;
