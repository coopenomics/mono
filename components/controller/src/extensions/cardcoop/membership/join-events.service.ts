/**
 * Приём в пайщики: выпуск свидетельства по связи, ждавшей решения совета (story 7.5, FR-E5).
 *
 ...
 *
 * Поэтому связь ждёт, а выпуск происходит здесь — по записи цепи о приёме. Дата берётся из
...
export class CardcoopJoinEventsService {
   ...
   *
   * Отсутствие ожидающей связи — норма и молчит: большинство принимаемых карту не
  ...
  async handleMemberAdded(actionData: InnerChainActionRecord): Promise<void> {
    ...
    try {
      ...
      // подтверждённого членства. Разбирается по журналу: связь не потеряна, она ждёт.
