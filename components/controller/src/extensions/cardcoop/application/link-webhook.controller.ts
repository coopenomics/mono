...
export class CardcoopLinkWebhookController {
   ...
   * решение владельца от 01.09.2026, а не упущение. Блокировка (`soviet::block`, в том
   * числе через отмену регистрации) снимает право голоса, но строку из реестра не убирает:
   * человек остаётся пайщиком, а блокировка — дело кооператива, а не карты. Свидетельство
   * при этом утверждает единственное — что человек состоит с такой-то даты, и о правах
   * внутри кооператива не говорит ничего. Прекращает членство только выход, и узнаём мы о
   * нём не по статусу, а по исчезновению строки (`soviet::delpartcpnt` внутри завершения
   * выхода), которое расширение ловит событием `completexit`.
   *
