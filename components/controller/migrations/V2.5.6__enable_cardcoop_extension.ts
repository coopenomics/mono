...
/**
 * Расширение «Карта кооператора» включается у уже установленных кооперативов (3B5-65).
 *
...
export default {
  ...
  async up({ dataSource, logger }: { dataSource: DataSource; logger: MigrationLogger }): Promise<boolean> {
    ...
    try {
      ...
      logger.info(
        updated.length > 0
          ? 'Расширение «Карта кооператора» включено у кооператива'
          : 'Расширение «Карта кооператора» уже включено либо ещё не установлено — ничего не менялось'
      );
      return true;
    } catch (error) {
