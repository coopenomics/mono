 ...
 * `w.wal.share[username]` (USER_SHARED) на биллинг-кошелёк кооператива
 * `w.wal.bill[coopname]` (COOPERATIVE, Epic 13 v5.1): паевой взнос
 * (возвратный) превращается в членский (целевой, невозвратный), зарезервированный
 * под оплату подписок (как time-, так и package-вариантов). Сам факт конвертации
 * трактуется как согласие пайщика на последующие рекуррентные списания (см.
 * `pay` для time-подписок и `topup_axon` для пакетных).
 *
 * @param coopname     Кооператив-плательщик (его подписью релеит бэкенд после JWT)
 *                     и сразу же scope биллинг-кошелька назначения.
 * @param username     Пайщик, чей паевой конвертируется (L3-снятие с `w.wal.share`).
