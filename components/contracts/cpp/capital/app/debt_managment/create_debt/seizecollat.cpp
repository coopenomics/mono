/**
 * @brief Закрывает заём изъятием коммитов-обеспечения в НМА кооператива.
 *
 ...
 * (`w.cap.gen`), изымаются в НМА кооператива (`w.cap.nma`), а обязательство
 * пайщика по займу списывается. Никаких санкций к пайщику не применяется,
 * совет не собирается, подпись пайщика не требуется.
 *
 * Двухоперационная свёртка ledger2:
 *  - o.cap.seize  — TRANSFER w.cap.gen → w.cap.nma, Дт 04 / Кт 08.
 *                   Зеркалит o.cap.accept, но без участия пайщика: НМА из
 *                   обеспечения идут в имущество кооператива, а не на share
 *                   программы пайщика.
 *  - o.cap.wroff  — BURN w.cap.loan, Дт 80 / Кт 58.
 *                   Закрывает финансовое вложение кооператива в пайщика.
 *                   Зеркало o.cap.repay, но share пайщика не пополняется —
 *                   платёж сделан имущественно через коммиты.
 *
 ...
 */
void capital::seizecollat(name coopname, checksum256 debt_hash) {
  ...
  eosio::check(exist_debt.status == Capital::Debts::Status::PAID
            || exist_debt.status == Capital::Debts::Status::OVERDUE,
               "Изъятие обеспечения допустимо для активного (paid) или просроченного (overdue) займа");
  ...
  eosio::check(project.status == Capital::Projects::Status::CANCELLED,
               "Изъятие обеспечения допустимо только при cancelled статусе компонента проекта");
  ...
  // 6. o.cap.seize: TRANSFER GENERATOR_FUND → NMA, Дт 04 / Кт 08.
  // Коммиты-обеспечение из «Генератора» пайщика уходят в НМА кооператива.
  Ledger2::apply(_capital, coopname, operations::capital::SEIZE_COLLATERAL,
                 exist_debt.amount, exist_debt.username, debt_hash, memo);

  // 7. o.cap.wroff: BURN LOAN_ISSUED, Дт 80 / Кт 58.
  // Финансовое вложение кооператива в пайщика закрыто; share пайщика не
  // пополняется — займ погашен имущественно.
  Ledger2::apply(_capital, coopname, operations::capital::WRITE_OFF_LOAN,
                 exist_debt.amount, exist_debt.username, debt_hash, memo);
  ...
  // 11. Запись долга → SEIZED (для аудита).
  Capital::Debts::mark_seized(coopname, exist_debt.id);
