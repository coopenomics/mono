...
namespace Capital {
   ...
   */
  // Значения по умолчанию держатся В СООТВЕТСТВИИ с defaultConfig бэкенда
  // (controller/src/extensions/capital/capital-extension.module.ts): именно он
  // задаёт конфигурацию кооперативу при онбординге, а эти значения работают
  // ровно до неё. Пока они расходились, кооператив без явной настройки жил по
  // другим правилам, чем показывал интерфейс.
  struct config {
    double coordinator_bonus_percent = 5;   ///< Процент премий координатора от инвестиций
    double expense_pool_percent = 100;         ///< Процент инвестиций в пул расходов
    uint32_t coordinator_invite_validity_days = 30; ///< Срок действия приглашения координатора (дней)
    uint32_t voting_period_in_days = 1;        ///< Период голосования в днях
    double authors_voting_percent = 62.8;      ///< Процент премий авторов для голосования
    double creators_voting_percent = 62.8;     ///< Процент премий создателей для голосования

    // Параметры геймификации
    double energy_decay_rate_per_day = 0.02;  ///< Скорость уменьшения энергии в день
    uint64_t level_depth_base = 100000000;     ///< Базовая сумма вкладов для уровня 1 (10000 RUB при точности рубля 4 знака: 10000 × 10^4)
