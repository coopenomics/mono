...
\brief Авторизация сегмента "contribute" для направления deliver_on_order (authordcont).
...
*/
[[eosio::action]] void marketplace::authordcont(eosio::name coopname, checksum256 request_hash, document2 authorization) {
  ...
  eosio::check(change.type == "offer"_n, "Метод authordcont применим только к заявкам типа offer");
...
/**
\ingroup public_actions
\brief Авторизация сегмента "return" для направления deliver_on_order (authordret).

@details Метод используется для подтверждения согласия совета на заявление на конвертацию в направлении ORDER → OFFER.
Заявка становится авторизованной только когда оба сегмента (contribute и return) получили авторизацию.
...
@note Авторизация требуется от аккаунта: @p _soviet
*/
[[eosio::action]] void marketplace::authordret(eosio::name coopname, checksum256 request_hash, document2 authorization) {
  require_auth(_soviet);
  ...
  eosio::check(change.type == "offer"_n, "Метод authordret применим только к заявкам типа offer");
  eosio::check(change.parent_id > 0, "У встречной заявки должна быть родительская заявка");
