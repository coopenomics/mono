...
\brief Авторизация сегмента "return" для направления deliver_on_offer (authoffc2r).
...
*/
[[eosio::action]] void marketplace::authoffc2r(eosio::name coopname, checksum256 request_hash, document2 authorization) {
  ...
  eosio::check(change.type == "order"_n, "Метод authoffc2r применим только к заявкам типа order");
