...
\brief Авторизация сегмента "contribute" для направления deliver_on_offer (authoffs2c).
...
*/
[[eosio::action]] void marketplace::authoffs2c(eosio::name coopname, checksum256 request_hash, document2 authorization) {
  ...
  eosio::check(change.type == "order"_n, "Метод authoffs2c применим только к заявкам типа order");
