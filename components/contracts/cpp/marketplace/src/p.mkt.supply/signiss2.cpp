...
void marketplace::signiss2(eosio::name coopname,
  ...
  // дособирается с уже внесённого членского (o.mkt.lockmf), затем 100% фактической суммы взноса
  ...
  if (locked_fee.amount > 0) {
    ...
    if (fact_fee < locked_fee) {
      ...
      // Факт больше заказа: недостающая часть ЧЛЕНСКОГО ВЗНОСА (комиссии КУ)
      // дособирается с уже внесённого членского (o.mkt.lockmf: w.mkt.member →
      // w.mkt.fee, без проводки), НЕ с паевого. Источник членского пополняется
      // заранее Заявлением о конвертации; здесь — проверка членского.
