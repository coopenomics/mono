...
struct ledger2_wallets {
  ...
  // marketplace — программный членский + резерв под Order + выплаты
  static constexpr eosio::name MARKETPLACE_MEMBER     = "w.mkt.member"_n;  ///< ЦПП «Стол Заказов» — программный членский кошелёк пайщика (USER_SHARED; .available = свободные средства программы)
  static constexpr eosio::name MARKETPLACE_ORDER_LOCK = "w.mkt.order"_n;   ///< ЦПП «Стол Заказов» — резерв средств пайщика под конкретный Order (USER_SHARED). TRANSFER w.mkt.member → w.mkt.order на createorder; обратный TRANSFER на cancel/decline/expire; BURN с w.mkt.order на signiss2 (выдача имущества). Заменяет механику BLOCK/UNBLOCK на w.mkt.member.blocked 2026-05-28.
