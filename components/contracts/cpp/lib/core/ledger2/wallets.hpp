...
struct ledger2_wallets {
  ...
  static constexpr eosio::name MARKETPLACE_SHARE_FUND = "w.mkt.share"_n;   ///< ЦПП «Стол Заказов» — свободный паевой пайщика в программе (USER_SHARED, счёт 80). Сюда возвращается остаток резерва при отмене/недовыдаче и гарантийном возврате; отсюда фондируются заказ из остатка и доплата по факту (o.mkt.lockp) и конвертируется по заявлению недостающая часть взноса участка (o.mkt.convp); выводится в общий паевой действием recallshare (o.mkt.recall).
  static constexpr eosio::name MARKETPLACE_MEMBER_FUND = "w.mkt.member"_n; ///< ЦПП «Стол Заказов» — внутренний членский кошелёк пайщика в программе (USER_SHARED, счёт 86). Пополняется по Заявлению о переводе 1110 (действие convert: o.mkt.conv с w.wal.share, o.mkt.convp с w.mkt.share) на недостающую часть и возвратами членских средств (сторно взноса o.mkt.refund, снятие членского резерва o.mkt.unlkm, гарантийный возврат o.mkt.retm). Расходуется первым при каждом заказе: на взнос участка (o.mkt.fee → w.mkt.fee) и на само тело заказа (o.mkt.lockm → w.mkt.morder). Членский: обратно в паевой не транслируется.
  static constexpr eosio::name MARKETPLACE_MEMBER_LOCK = "w.mkt.morder"_n; ///< ЦПП «Стол Заказов» — членский резерв пайщика под конкретный Order (USER_SHARED, счёт 86): часть тела заказа, оплаченная из внутреннего членского кошелька. TRANSFER w.mkt.member → w.mkt.morder на createorder/stockorder (o.mkt.lockm); обратно на w.mkt.member при отмене и недовыдаче (o.mkt.unlkm); BURN на issueact2 (o.mkt.consm, Дт 86 / Кт 10); удержание при отказе — o.mkt.penalm в пул взносов.
...
inline constexpr std::array<Ledger2WalletMeta, 28> LEDGER2_WALLET_REGISTRY = {{
  // USER_SHARED (13) — L3-разрез по пайщику (у w.brn.common — по braname КУ)
  ...
  { ledger2_wallets::MARKETPLACE_MEMBER_LOCK,"ЦПП «Стол Заказов» — членский резерв под заказ у пайщика", WalletKind::USER_SHARED },
...
inline constexpr std::array<Ledger2WalletProgramMapping, 14> LEDGER2_USER_SHARED_PROGRAM_MAPPING = {{
  ...
  { ledger2_wallets::MARKETPLACE_MEMBER_LOCK, 2 /* Marketplace */                   },
 ...
 *
 * НЕ входят: w.wal.member, w.mkt.member и w.mkt.morder (членские — невозвратные), w.exp.adv (подотчёт под
