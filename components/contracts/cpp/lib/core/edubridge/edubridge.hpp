...
namespace Memo {
  ...
  inline std::string get_lock_fee_memo() {
    return "Удержание членского взноса до конца гарантийного срока курса по ЦПП «Образование»";
  }

  inline std::string get_unlock_fee_memo() {
    return "Разблокировка членского взноса по ЦПП «Образование»: гарантийный срок истёк либо подписка закрыта";
  }
