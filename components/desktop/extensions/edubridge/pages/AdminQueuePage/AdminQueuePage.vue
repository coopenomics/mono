<template lang="pug">
.q-pa-md
  PageHint(storage-key="edu:admin-queue:banner-dismissed")
  ...
  BaseTable(:columns="columns" :rows="items" row-key="id" :loading="loading && !items.length" min-width="760px")
    ...
    template(#cell-next_attempt_at="{ row }") {{ row.status === 'pending' ? formatDateTime(row.next_attempt_at) : '—' }}
    template(#cell-actions="{ row }")
      BaseButton(v-if="row.status === 'needs_attention' || row.status === 'failed'" variant="secondary" size="sm" :loading="busy === row.id" @click="onRetry(row)") Повторить
...
async function load(): Promise<void> {
  loading.value = true;
  try {
    const statuses = tab.value === 'attention' ? ['needs_attention', 'failed'] : tab.value === 'active' ? ['pending', 'running'] : undefined;
