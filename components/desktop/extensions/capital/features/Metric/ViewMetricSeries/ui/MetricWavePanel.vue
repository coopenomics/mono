...
.metric-wave(v-if='wave')
  .metric-wave__card-head
    .metric-wave__titles
      .metric-wave__label Волна
      .metric-wave__hint Изменения по периодам и разметка
    .metric-wave__phase
      span.metric-wave__badge {{ displayLabel }}
      span.metric-wave__phase-label {{ phaseLabel }}
      span.metric-wave__eta.t-mono(
        v-if='wave.corridor.eta_base_periods != null'
      ) ~{{ wave.corridor.eta_base_periods }} пер.
  ...
  .metric-wave__bars(v-if='deltas.length')
    .metric-wave__bar-col(
      v-for='(d, i) in deltas',
      :key='i'
    )
      .metric-wave__bar-track
        .metric-wave__bar(
          :style='barStyle(d)',
          :class='{ "metric-wave__bar--neg": d < 0 }'
        )
      .metric-wave__bar-val.t-mono {{ formatDelta(d) }}
...
const maxAbsDelta = computed(() =>
  Math.max(...props.deltas.map((d) => Math.abs(d)), 1),
);

const barStyle = (delta: number) => {
  const h = Math.max(
    (Math.abs(delta) / maxAbsDelta.value) * 100,
    delta === 0 ? 2 : 10,
  );
  return { height: `${h}%` };
};
...
.metric-wave__bar--neg {
  background: var(--p-neg);
}

.metric-wave__bar-val {
  font-size: 9px;
  color: var(--p-ink-3);
  line-height: 1;
}
