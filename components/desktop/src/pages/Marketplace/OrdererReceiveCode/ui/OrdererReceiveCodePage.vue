...
q-page.receive(role="region", aria-label="Получить заказ")
  .receive__inner(v-if="receiveCode")
    HandoffQr(
      :value="receiveCode",
      :size="320",
      caption="Покажите этот код оператору на том пункте выдачи, куда пришёл ваш заказ — он выдаст по нему всё, что готово к получению. Код можно показать заранее или с распечатки."
    )
  EmptyState(
    v-else,
