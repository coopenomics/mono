export interface IWorkspaceRouteMeta {
  ...
  hidden?: boolean
  // Какой пункт меню подсвечивать на скрытой странице. Карточка курса, заказа,
  // предложения — сосед реестра, а не его потомок, поэтому раздел указывается
  // явно; без него подсветка гаснет, пока человек смотрит карточку.
  menuKey?: string
  [key: string]: any
}
