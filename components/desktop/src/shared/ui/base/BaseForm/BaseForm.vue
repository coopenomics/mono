...
/* Когда у поля есть подсказка или ошибка, текст занимает весь
   зарезервированный запас, и следующий элемент начинается сразу под ним:
   «Минимум 8 символов» читалось как подпись к полю ниже, а «Пароли не
   совпадают» упиралось в кнопку. Зазор разводит их и работает в обе стороны —
   и между полями, и между последним полем и кнопкой.
   Инвариант «поля не прыгают» сохраняется: место под подсказку резервируется
   всегда, поэтому появление текста ничего не сдвигает. */
