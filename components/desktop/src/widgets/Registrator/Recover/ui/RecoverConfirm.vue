...
async function enterDesktop(): Promise<void> {
  ...
  // Сбой загрузки контекста и «пайщик ещё не вступил» — РАЗНЫЕ вещи, и путать их
  // нельзя. `isRegistrationComplete` — это просто «в контексте есть учётная запись
  // пайщика»; когда контекст не приехал, признак тоже false, и принятого пайщика
  // уносило на «Вступить в пайщики» поверх его же кабинета. Ровно это увидел
  // `pgrzosdeyuwg` 08.09.2026: слева меню Стола Пайщика, по центру — предложение
  // вступить, а сам он в цепи принят ещё в 2024 году. Поэтому ошибку не глотаем:
  // не смогли спросить, кто перед нами, — отправляем на вход, а не в регистрацию.
  try {
