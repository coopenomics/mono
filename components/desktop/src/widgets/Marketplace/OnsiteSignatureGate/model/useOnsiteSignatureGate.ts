...
/**
 * Глобальный гейт «подпись на месте» (Фаза 1, на poll-запросах).
 *
 ...
 *
 * Сигнал = сам статус (никакого нового события не нужно). Источник — тот же
 * poll, что и столы; гейт сам закрывается, как только статус ушёл вперёд
 * (подписал) ИЛИ оператор откатил (на следующем тике список пустеет).
 *
 * Фаза 2 (#56/#57) заменит poll на GraphQL-подписку и реактивный стор.
 */
