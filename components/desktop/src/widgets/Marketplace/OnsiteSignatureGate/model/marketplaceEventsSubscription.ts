    ...
    open() {
      ...
      return {
        ...
        close: () => {
          ...
          // Закрываем ТОЛЬКО свою подписку. Раньше здесь гасился общий
          // транспорт (`client.disposeSubscriptions()`), чтобы оборвать
          // зависший цикл переподключения, — но транспорт общий: вместе с ним
          // умирали подписки кошелька и состояния узла, а внутренний промис
          // подключения отклонялся событием закрытия и уезжал в журнал ошибок
          // висячим отказом (инцидент 17.09.2026). Зависший цикл теперь
          // обрывается иначе: ядро переоткрывает мёртвую подписку, а отказы
          // транспорта разбирает его постоянный слушатель в SDK.
          stream.ws.close();
        },
      };
    },
