...
BaseDialog(
  ...
  p.convert-billing__lead
    | Паевой взнос по программе «Цифровой Кошелёк» переводится в членский взнос
    | по соглашению о подключении — с него оплачиваются подписки кооператива.
...
const onSubmit = async () => {
  ...
  try {
    await convert()
    SuccessAlert('Кошелёк пополнен: паевой взнос переведён в членский')
  } catch (e: any) {
    FailAlert(e)
  }
}
