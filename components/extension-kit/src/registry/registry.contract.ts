...
export interface IRegistryExtension {
  ...
  /**
   * Capability-заявка расширения: какие порты контура оно просит (ADR-16).
   *
   * Это не права пайщика — те проверяются на границе API самого расширения,
   * внутри уже авторизованного сценария. Здесь другой вопрос: что этому
   * расширению вообще позволено просить у кооператива. Ответ на него нужен
   * один раз, при запуске, а не на каждом вызове порта.
   *
   * Обязательный порт без реализации — отказ на старте с внятной причиной.
   * Необязательный может отсутствовать: расширение работает, часть
   * возможностей выключена.
   */
  ports?: {
    required: ReadonlyArray<symbol>;
    optional: ReadonlyArray<symbol>;
  };
  tags?: string[];
