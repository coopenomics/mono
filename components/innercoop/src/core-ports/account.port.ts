...
export interface IAccountPort {
  ...
  /**
   * Запись аккаунта в цепи — то, что знает о пайщике сама цепь, включая дату
   * приёма. Отличается от учётной записи: там данные кооператива, здесь —
   * состояние в цепи. `null`, если аккаунт в цепи не заведён.
   */
  getChainAccount(username: string): Promise<InnerChainAccountRow | null>;
}

/**
 * Строка аккаунта в цепи. Перечислено то, что читают расширения; остальное
 * доступно через индекс — форму задаёт контракт регистратора.
 */
export interface InnerChainAccountRow {
  username?: string;
  /** Момент приёма в кооператив, как его записала цепь. */
