...
export interface IUserDirectoryPort {
   ...
   * хранилища, у новых это обычный uuid. Подходит и `sub` из CoopID (uuid учётки
   * authentik): ядро само сводит его к пайщику через имя учётки. Расширение про это
   * знать не должно, поэтому разбор живёт здесь. Бросает, если идентификатор не
   * разобран или пользователя нет.
   */
