...
| `CHAIN_PORT` | `IChainPort` (4)<br><sub>core-ports/chain.port.ts</sub> | `ChainInnercoopAdapter` | capital, chairman, expenses, ku, marketplace | Проводка действий в цепь и чтение её таблиц. |
...
| `LOGGER_PORT` | `ILoggerPort` (6)<br><sub>core-ports/logger.port.ts</sub> | `WinstonLoggerService` | builtin, capital, chairman, chatcoop, expenses, ku, marketplace, participant, powerup, qrpay, sberpoll, yookassa | Журналирование контура кооператива. |
...
| `INDIVIDUAL_PORT` | `IOrganizationPort` (2)<br><sub>core-ports/party-card.port.ts</sub> | `IndividualInnercoopAdapter` | ku | Карточки сторон — реквизиты организации и данные физического лица. |
| `PAYMENT_DESK_PORT` | `IPaymentDeskPort` (3)<br><sub>core-ports/payment-desk.port.ts</sub> | `PaymentDeskInnercoopAdapter` | marketplace | Кассирский стол — операции над платежом, а не записи о нём. |
...
| `VAULT_PORT` | `IVaultPort` (1)<br><sub>core-ports/vault.port.ts</sub> | `VaultInnercoopAdapter` | capital, chairman, expenses, ku, marketplace | Ключи подписи, хранимые кооперативом. |
