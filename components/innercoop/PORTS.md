...
| `NOTIFICATION_PORT` | `INotificationPort` (2)<br><sub>core-ports/notification.port.ts</sub> | `NotificationInnercoopAdapter` | chairman, chatcoop, edubridge, expenses, ku, marketplace, participant | Центр уведомлений: расширение говорит, ЧТО и КОМУ отправить, не зная про каналы, шаблоны и провайдеров. |
...
| `USER_DIRECTORY_PORT` | `IUserDirectoryPort` (3)<br><sub>core-ports/user-directory.port.ts</sub> | `UserDirectoryInnercoopAdapter` | capital, chatcoop, edubridge, marketplace | Справочник пользователей кооператива — учётные имена и роли. |
