...
export default async ({ page, shot, expect }) => {
  ...
  await shot(
    page,
    "02-offer-detail",
    "Карточка предложения открывается по клику из каталога: фотографии, полное описание, цена за единицу с учётом целевого членского взноса, поставщик и условия поставки на выбранный пункт выдачи.",
    {
      expect: async (p) => {
