...
запуск без правок документов не отправляет ни одной транзакции.
...
В раскатке сети команду запускает playbook деплоя контрактов
(`coopenomics/playbooks`, `contracts/setup-contracts.yaml`) образом
`dicoop/bootcoop:<branch>` сразу после установки контрактов. Вручную:

```bash
CHAIN_URL=https://api.coopenomics.world \
EOSIO_PRV_KEY=<ключ системного аккаунта> \
pnpm -F @coopenomics/boot run drafts:sync -- --dry-run
```

`--dry-run` печатает план и не отправляет транзакций; ключ в этом режиме не
нужен. Реестр правит системный аккаунт — `draft::createdraft` со `scope=draft`
