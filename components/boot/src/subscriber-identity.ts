 ...
 * Без SERVER_SECRET адрес не заводится — его дозаполнит контроллер при старте.
 */
export function subscriberIdentity(coopname: string): { subscriber_id: string, subscriber_hash: string } | { subscriber_id: null, subscriber_hash: null } {
  const secret = process.env.SERVER_SECRET
  if (!secret)
    return { subscriber_id: null, subscriber_hash: null }
