...
//
// АПП приёмки имущества ОТ ПОСТАВЩИКА В КООПЕРАТИВ (поставщик → кооператив).
// Это НЕ акт выдачи заказчику (тот — 1105.MarketplaceAplIssuance). Заказчик в
// этом акте стороной не является.
//
// Семантика подписантов:
//   - `username` (из IGenerate) — USERNAME ПОСТАВЩИКА (передающая сторона,
//     «Передал»). ФИО/наименование подставляется factory через
//     `getUser(username) → user`.
//   - `transmitter` — USERNAME оператора кооперативного участка (председатель
//     КУ или доверенное им лицо), принимающего имущество от лица Кооператива
//     («Получил»). ФИО подставляется factory через
//     `getUser(transmitter) → getFirstLastMiddleName`.
//
// В Action входят как переменные шаблона (order_id/act_id/transmitter/braname),
// так и инфраструктурные признаки записи АПП (reception_id/accept_braname/
// fact_quantity/total_amount/supplier_account) — последние не подставляются в
// текст, но обязаны попасть в meta подписанного документа, чтобы on-chain акт
// был самоописывающимся. meta генерируется фабрикой как `{...data}`, поэтому
// любое поле Action автоматически оказывается в meta.
//
// `doc_data_hash` — зарезервированное опциональное поле для приватных данных
// off-chain (см. раздел «Document Generation Pattern: doc_data»).
export interface Action extends IGenerate, IDocDataRef {
  ...
  act_id: string
  /** USERNAME оператора КУ (принимающая сторона от Кооператива). */
  transmitter: string
  ...
  total_amount: string
  /** USERNAME поставщика, передавшего партию (передающая сторона). */
...
// Модель данных документа. `decision` не используется в тексте (приёмка не
// сопровождается протоколом совета), но сохранён в модели для совместимости с
// общей фабрикой (передаётся заглушкой).
...
export const translations = {
  ru: {
    ...
    act_number: 'АКТ №{0}',
    act_name: 'приёмки-передачи имущества (приём в кооператив) по Целевой Потребительской Программе «{0}»',
    contribution_act: '{0} "{1}" (далее – Кооператив), в лице полномочного Представителя Кооператива, и поставщик {2} (далее – "Поставщик") составили настоящий Акт о том, что Поставщик передал, а Кооператив получил от Поставщика, в соответствии с Целевой Потребительской Программой "{3}", следующее имущество:',
    branched_contribution_act: 'Кооперативный участок "{0}" {1} "{2}" (далее – Кооператив), в лице полномочного Представителя Кооператива, и поставщик {3} (далее – "Поставщик") составили настоящий Акт о том, что Поставщик передал, а Кооператив получил от Поставщика, в соответствии с Целевой Потребительской Программой "{4}", следующее имущество:',
    ...
    signature: 'Подпись',
    participant_full_name: 'ФИО/Наименование',
  },
}
...
export const exampleData = {
  ...
  user: {
    full_name_or_short_name: 'ООО «Молокозавод»',
  },
  request: {
