...
export const registry_id = 328

/**
 * Интерфейс генерации договора о полной индивидуальной материальной
 * ответственности председателя кооперативного участка. Стороны договора:
 * «Общество» — кооператив в лице Председателя Совета (накладывает вторую
 * подпись после решения совета об учреждении участка) и «Исполнитель» —
 * избранный председатель кооперативного участка (подписывает договор при
 * подаче заявления о создании участка). Документ подписывается двумя сторонами:
...
export const title = 'Договор о полной индивидуальной материальной ответственности председателя кооперативного участка'
export const description = 'Форма договора о полной индивидуальной материальной ответственности председателя кооперативного участка между кооперативом в лице Председателя Совета и избранным председателем участка'
export const context = `<style>.digital-document h1 {margin: 0px;text-align:center;font-size: 16px;}.digital-document {padding: 20px;white-space: pre-wrap;}.digital-document p {margin: 0px;padding-top: 10px;text-align: justify;}.requisites p {padding-top: 2px;}.signature {padding-top: 20px;}</style><div class="digital-document"><h1 class="header">{% trans 'AGREEMENT_TITLE' %}</h1><p style="text-align: right; padding-top: 20px;">{{ coop.city }}, {{ meta.created_at }}</p><p>{% trans 'PREAMBLE', vars.full_abbr, vars.name, coop.chairman.last_name, coop.chairman.first_name, coop.chairman.middle_name, branch_name, vars.full_abbr, vars.name %}</p><p>{% trans 'CLAUSE_1' %}</p><p>{% trans 'CLAUSE_1_A' %}</p><p>{% trans 'CLAUSE_1_B' %}</p><p>{% trans 'CLAUSE_1_C' %}</p><p>{% trans 'CLAUSE_1_D' %}</p><p>{% trans 'CLAUSE_2' %}</p><p>{% trans 'CLAUSE_2_A' %}</p><p>{% trans 'CLAUSE_2_B' %}</p><p>{% trans 'CLAUSE_2_REVISION' %}</p><p>{% trans 'CLAUSE_3' %}</p><p>{% trans 'CLAUSE_3_PARA2' %}</p><p>{% trans 'CLAUSE_4' %}</p><p>{% trans 'CLAUSE_5' %}</p><p>{% trans 'COPIES' %}</p><div class="requisites" style="padding-top: 30px;"><p><strong>{% trans 'SOCIETY_LABEL' %} {{ vars.full_abbr }} «{{ vars.name }}»</strong></p><p>{% trans 'INN' %} {{ coop.details.inn }}</p><p>{% trans 'KPP' %} {{ coop.details.kpp }}</p><p>{% trans 'OGRN' %} {{ coop.details.ogrn }}</p><p>{% trans 'LEGAL_ADDRESS' %} {{ coop.full_address }}</p><p>{% trans 'COUNCIL_CHAIRMAN_LABEL' %} {{ coop.chairman.last_name }} {{ coop.chairman.first_name }} {{ coop.chairman.middle_name }}</p><p>{% trans 'SIGNED_DIGITALLY' %}</p></div><div class="requisites" style="padding-top: 20px;"><p><strong>{% trans 'EXECUTOR_LABEL' %} {{ individual.last_name }} {{ individual.first_name }} {{ individual.middle_name }}</strong></p>{% if vars.passport_request == 'yes' %}<p>{% trans 'PASSPORT_SERIES_NUMBER', individual.passport.series, individual.passport.number %}</p><p>{% trans 'PASSPORT_ISSUED', individual.passport.issued_by, individual.passport.code %}</p><p>{% trans 'PASSPORT_ISSUED_AT', individual.passport.issued_at %}</p>{% endif %}<p>{% trans 'REGISTRATION_ADDRESS' %} {{ individual.full_address }}</p><p>{% trans 'CONTACT_PHONE' %} {{ individual.phone }}</p><p>{% trans 'SIGNED_DIGITALLY' %}</p></div></div>`

export const translations = {
  ru: {
    AGREEMENT_TITLE: 'ДОГОВОР О ПОЛНОЙ ИНДИВИДУАЛЬНОЙ МАТЕРИАЛЬНОЙ ОТВЕТСТВЕННОСТИ',
    PREAMBLE: 'В целях обеспечения сохранности материальных ценностей, {0} «{1}», в лице Председателя Совета {2} {3} {4}, действующего на основании Устава, именуемое в дальнейшем «Общество», и Председатель Кооперативного участка «{5}» {6} «{7}», именуемый в дальнейшем «Исполнитель», заключили настоящий договор о нижеследующем:',
    ...
    EXECUTOR_LABEL: 'Исполнитель: ФИО (полностью)',
