...
/**
 * Протокол решения совета о принятии имущества и восстановлении паевого
 * взноса — гарантийный возврат в паевой модели ЦПП «Стол заказов»
 * (компонент 68, задача 99D-9).
 *
 * Документ-решение совета по Заявлению 1116. Подписывается председателем
 * совета по типовому процессу решений совета — обычно роботом решений
 * совета. `soviet::exec` вызывает `marketplace::onmktrtauth(coopname, hash,
 * authorization)` с этим протоколом: все движения по выданному заказу
 * откатываются, паевой взнос восстанавливается на свободном паевом «Стола
 * заказов».
 */
export interface Action extends IGenerate {
  ...
  decision_id: number
  /** Канонический order_hash исходного заказа (из заявления 1116). */
  order_hash: string
  ...
  /** Цена за единицу. */
  unit_cost: string
  /** Стоимость возвращаемого имущества — размер восстанавливаемого паевого взноса. */
  fact_cost: string
  /** Символ валюты. */
  currency: string
}
...
export const title = 'Протокол решения совета о принятии имущества и восстановлении паевого взноса'
export const description = 'Форма протокола решения совета о принятии имущества в качестве паевого взноса при гарантийном возврате по ЦПП «Стол заказов»'
...
export const translations = {
  ru: {
    ...
    'question_number_one': 'Принять от пайщика Кооператива в качестве паевого взноса имущество согласно поданного заявления о гарантийном возврате по условиям Целевой Потребительской Программы "{0}" и восстановить его паевой взнос в размере стоимости принятого имущества, а именно:',
    ...
    'decision_number_one': 'Принять от пайщика {0} указанное в заявлении имущество в качестве паевого взноса и восстановить его паевой взнос в размере стоимости принятого имущества.',
  },
}
