 ...
 *
 * Документ-Решение совета по Заявлению о списании (registry 1106).
 * Подписывается председателем после голосования через стандартный
 * sov.decision-flow (`soviet::authorize` с этим документом в качестве
 * `authorization`). `soviet::exec` запускает callback
 * `marketplace::onmktwoauth(coopname, hash, authorization)`, который
 * сохраняет этот Протокол в `wroffprops.protocol` и переводит проект в
 * AUTHORIZED. Backend циклом по items[] вызывает `marketplace::execwroff`
 * per-item.
 */
export interface Action extends IGenerate {
  ...
  decision_id: number
  /** Канонический proposal_hash on-chain. */
  proposal_hash: string
  /** Идентификатор расчётного цикла списания (ISO-дата начала цикла). */
  cycle_started_at: string
  /** Σ items.amount — для шапки протокола. */
  total_amount: string
  /** Число позиций к списанию. */
  items_count: number
}
...
export const title = 'Протокол совета о списании скоропорта'
export const description = 'Форма протокола решения совета о списании имущества со складов кооперативных участков по ЦПП «Стол заказов»'
  ...
  <table>
    <tbody>
      ...
      <tr>
        ...
        <td>{% trans 'agenda_subject', items_count, total_amount, cycle_started_at %}</td>
      </tr>
    </tbody>
  </table>
  ...
  <table>
    <tbody>
      ...
      <tr>
        ...
        <td>{% trans 'decision_writeoff_text', items_count, total_amount %}</td>
      </tr>
    </tbody>
  </table>
...
export const translations = {
  ru: {
    ...
    agenda_subject: 'Утвердить списание {0} позиций имущества со складов кооперативных участков на общую сумму {1} (цикл от {2}) по ЦПП «Стол заказов» — поданное Заявление председателя о списании скоропорта.',
    decision_writeoff_text: 'Списать {0} позиций имущества на общую сумму {1} с балансов кооперативных участков через транзит счёта 91 «Прочие доходы и расходы» с закрытием на счёт 86 Целевого финансирования программы. Движений по кошелькам пайщиков не производить.',
  },
}
