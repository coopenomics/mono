...
### ПРОВЕРКА ТИПОВ ПО ТРОНУТЫМ ПАКЕТАМ — ПЕРЕД КАЖДЫМ КОММИТОМ

```
pnpm typecheck:changed
```

Ни eslint, ни `pnpm check` код не компилируют, а сборка пакетов (unbuild →
esbuild) типы просто срезает: `pnpm build` пройдёт зелёным на коде, который
падает в рантайме. CI (`typecheck.yaml`) проверяет только desktop и controller
и только на ветках dev/testnet/main, а зеркало Gitea → GitHub обновляется раз в
8 часов — как обратная связь в работе это не годится.

Гейт гоняет `tsc --noEmit` только в пакетах, которых коснулась ветка, и работает
храповиком: вердикт роняет РОСТ числа ошибок против
`scripts/lib/typecheck-baseline.json`. Починил старую — опусти число, поднимать
нельзя. Цена одного пакета: cooptypes 4 с, boot 12 с, factory 16 с, sdk 20 с,
desktop 38 с, controller 66 с.

desktop проверяется голым `tsc`: он не разбирает `<template>` и не видит типы,
экспортируемые из `.vue` (TS2614 — это артефакт инструмента, а не ошибки).
Ошибку внутри `<script setup>` он ловит, разметку — нет; её проверяет vue-tsc в
CI. Локально vue-tsc не гонять: на живом стенде он конкурирует с `quasar dev` и
висит часами.

**Кейс 07.09.2026.** За день на стенд уехали две ошибки типов: фабрика собирала
документ 1110 без обязательного поля модели (`TS2741`), а селектор SDK
спрашивал у схемы несуществующее поле акта (`TS2322`). Обе ловятся компилятором
за 16 и 20 секунд соответственно — но именно `factory` и `sdk` не проверяет ни
один CI-workflow. Владелец увидел их в браузере вместо меня.
...
| `pnpm typecheck:changed` | **tsc по тронутым пакетам, храповиком — перед каждым коммитом** |
| `pnpm typecheck:all` | то же по всем пакетам (≈2,5 мин) |
