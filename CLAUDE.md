...
любой прогон забирает CPU целиком и всё встаёт. Запрет действует и на «свободный»
стенд: он на том же железе.

Проверка типов при этом обязательна — просто она делается не здесь:
