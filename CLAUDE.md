...
`ci/tests` слушают три workflow: `typecheck`, `check` и `test`. Пуш
форсированный: полигон — не история, а стенд.

Когда пушить: не на каждый коммит (раннеры не бесплатны), а **по завершении
куска работы, до слияния**. Дождаться исхода и разобрать красное — часть
работы, а не «посмотрю потом»: незакрытый прогон означает, что ветка не
проверена.
...
07.09.2026 CI-проверка знала только desktop и controller — и через эту дыру на
стенд уехали две ошибки компилятора в factory и sdk, которые ловятся за
секунды. Теперь `typecheck` покрывает ещё factory и sdk; cooptypes и boot
подключаются сразу после слияния ветки паевой модели, где вычищен их долг.
