...
## Харнес: гейты, тесты, реестр покрытия

Введено 2026-08-05. Смысл — заменить обещания «я проверил» на команды с кодом возврата.

### Одна команда — один вердикт

`pnpm check` — четыре гейта, exit 0/1. Подробности в шапке `scripts/check.sh`.

| ярус | охват | что проверяет | требует ли чинить легаси |
|---|---|---|---|
| A «границы» | вся база, жёстко | ядро не знает про расширения; расширения не ходят друг в друга напрямую — только через `inter` | нет, нарушений ноль |
| B «канон» | только изменённые файлы | сложность >10, функция >60 строк, вложенность >4, прямые `q-*` вместо обёрток канона, `status: string` вместо enum | нет, legacy не трогается |
| C «реестр» | только изменённые файлы | тронул код зарегистрированной фичи — обнови её файл в `test-registry/` | нет, области вне реестра — долг |

Отдельно: `check:boundaries`, `check:changed`, `check:registry`. База сравнения — `CHECK_BASE` (по умолчанию `origin/dev`).

**Ключевой принцип: гейт падает только от того, что объявлено гейтом.** Накопленные ошибки линта печатаются отдельной строкой как долг и вердикт не роняют. Иначе гейт был бы красным с первого дня и его перестали бы читать.

**Чего гейты НЕ делают.** Они защищают от забывчивости, а не от халтуры. Тест, написанный «чтобы позеленело», проходит их все. От этого защищает только `pnpm mutate:changed`.

### Тесты

| команда | что входит | нужна ли живая цепь |
|---|---|---|
| `test:unit` | cooptypes, parser, notifications, controller — 202 теста | нет |
| `test:component` | factory (нужен MongoDB) | нет |
| `test:ci` | unit + component | нет |
| `test:integration` | sdk, boot | **да** — нода, парсер, controller |

Юнит-тесты controller гоняются через `jest -i` (runInBand) — это разрешённая форма, запрет из раздела выше касается параллельного полного прогона.

### CI

**Actions работают на GitHub `coopenomics/mono`**, куда прилетает зеркало пушей из Gitea. В Gitea Actions отключены (`has_actions: false`).

⚠️ **PR на GitHub не создаются с 19.05.2026** — работа идёт в Gitea. Поэтому workflow на одном лишь `pull_request` **не запустится никогда**: ровно так `typecheck.yaml` не отработал ни разу за всё существование. Рабочий триггер — `push: branches: [dev]`.

- `check.yaml` — гейты, на push в dev. Без `lerna run build`: правила не типозависимые.
- `test.yaml` — только `workflow_call` + `workflow_dispatch`. Полный прогон на каждый push избыточен.
- `release.yaml` — job `release` объявлен `needs: tests`. **Красные тесты останавливают релиз** до сборки образов, публикации пакетов и вебхука деплоя (всё это `needs: release`).

### Реестр покрытия и скилл `feature-tests`

`test-registry/<домен>.<фича>.yaml` — один файл на фичу, три уровня сразу (contract/backend/ui). Схема в `test-registry/README.md`.

- `pnpm registry:audit` — карта покрытия и, главное, **области кода вне реестра**. Это вычисляемый ответ на «чего у нас нет», а не составленный руками.
- Скилл `/feature-tests` — семь фаз: локализация фичи → источники истины → перечисление ветвей → слияние в реестр → тесты по уровням → мутации → отчёт. Работает и по недокументированным фичам: ветви перечисляются от кода, а не от PRD.

**Железное правило: не подгонять тест под текущее поведение.** Если из кода нельзя понять, как поведение *должно* работать, — помечать случай `decision_needed: true` и выносить человеку списком. Иначе тест зафиксирует баг как норму и будет защищать его от исправления.

### Мутации — единственная защита от бесполезных тестов

`pnpm mutate:changed` (Stryker, controller). Мутатор портит исходник и смотрит, покраснеет ли тест. Не покраснел — тест ничего не проверяет.

Только по изменённым файлам: прогон одного файла ~2 минуты, полный по 1930 файлам занял бы часы и потому не запускался бы никогда. В общий гейт намеренно НЕ включён — это инструмент работы по фиче.

Показательный замер на `livekit-room-mapping.util.ts`: 5 зелёных тестов, построчное покрытие 84%, **реальная защита 29%**. Выживший мутант `/=+$/` → `/=$/` означал, что base64 с двумя `=` в хвосте не проверяется ничем.

### Ловушки, на которые уже наступали

- **`build:lib` собирал 2 пакета из 3.** Стоял `--scope @coopenomics/cooptypes`, а пакет называется `cooptypes` **без скоупа**. Lerna молча не находила его и выходила с кодом 0; `cooptypes/dist` висел устаревшим месяцами. Симптом — `BadRequestException: Неизвестный operation_code` и подобные «рассинхрон реестра» на ровном месте.
- **Коды ledger2 пишутся с префиксами** `o.` / `p.` — так их эмитит контракт (`ledger2.hpp`) и так они лежат в `LEDGER2_OPERATION_REGISTRY`. Беспрефиксные коды прошлого поколения (`reg.entrfee`, `cap.act2shr`, `wall.depcpl`) в реестре отсутствуют.
- **Stryker и pnpm.** При строгом `node_modules` автопоиск плагинов по маске `@stryker-mutator/*` не находит ничего — падает с «Cannot find TestRunner plugin jest». Плагин прописан явно в `stryker.config.json`.
- **`pipefail` в конвейере `eslint | eslint-gate.mjs`** пропускает наружу код возврата самого eslint, и гейт падает от долга, который обязан игнорировать. Поэтому в `check.sh` и `lint-changed.sh` намеренно `set -u` без `pipefail`.
- **`pgrep`/`pkill` по строке, встречающейся в собственной команде,** матчат сами себя: `until ! pgrep -f "build:lib"` крутится вечно, `pkill -f '...'` убивает свою же задачу.
