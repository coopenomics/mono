...
**Поднятие версии само по себе переподписей больше не вызывает.** Пайщикам
кооператива предъявляется редакция, утверждённая его советом
(`draft::approvals`, область кооператива, — фабрика утверждений документов,
компонент 66 проекта 13, задачи A3D-*). После `upversion` председатель
получает уведомление, выносит редакцию на совет со вкладки «Шаблоны
документов» (реестр документов стола совета), и только после решения совета
`draft::approve` переключает кооператив на новую редакцию: генератор берёт
новый текст, контракты подписи пишут новую версию, `RequireAgreements`
просит переподписать. До решения генератор собирает текст утверждённой
редакции из реестра узла (`draft_templates`, последний блок этой версии), а
`agreementTemplates` отдаёт утверждённую версию. Служебные документы
(повестка, протоколы, бюллетени) утверждения не требуют.

Правила выпуска редакции:

- смысловое изменение → поднять версию в `versions.ts`; каждый кооператив
  утверждает её советом сам, отставание видно в реестре шаблонов;
- правка без смысла → только текст, она доезжает до кооперативов сразу, в том
  числе в утверждённую, но уже не текущую редакцию;
- релиз, включающий сидер утверждений (`DOCUMENT_APPROVAL_SEED_ON_START`,
  первый выкат фабрики), **не совмещать с поднятием версий** — иначе перенос
  утвердит редакцию, которую совет не видел;
- документы, которыми пользуется кооператив, объявляются ядром
  (`core-document-declarations.ts`) и расширениями через
  `DOCUMENT_DECLARATION_PORT`; не объявленный шаблон в реестре кооператива
  не появляется и утверждён быть не может. Шаблоны-двойники «для
  утверждения» (995, 997, 999, 1101) не объявлять: совет утверждает рабочий
  документ в бланке.

**Новый документ расширения заводится в трёх местах сразу.** Шаблон в
`cooptypes/src/cooperative/registry/`, фабрика в `factory` (действие, шаблон,
`Templates/registry.ts`, диспетчер в `src/index.ts`) — и **объявление в
реестре шаблонов кооператива**: расширение зовёт `registerDocuments` через
`DOCUMENT_DECLARATION_PORT` в своём `initialize()`
(`application/onboarding/register-<name>-documents.ts`, канон —
`register-marketplace-documents.ts`). Первые два места дают документ
генератору, третье — совету: **необъявленный шаблон в реестре кооператива не
появляется, и утвердить его редакцию нечем**. Положение и оферты наследуют
ключи шагов онбординга в `bundle`/`vars_field`, бланки собираются в один
`bundle`, протоколы объявляются `service` (утверждения не требуют).
Кейс 20.09.2026: у расширения `edubridge` объявления не было вовсе — двенадцать
шаблонов образования жили вне фабрики утверждений, и заметили это случайно.
...
## Новый кошелёк приложения — политика выхода из кооператива согласовывается

Выход пайщика обходит таблицу `EXIT_WALLET_POLICY`
(`contracts/cpp/lib/core/ledger2/exit_policy.hpp`): она говорит, что делать с
каждым кошельком — вернуть на главный паевой и какой операцией
(`RETURN_TO_MAIN`), оставить кооперативу (`FORFEIT`), держать выход до
закрытия обязательства (`BLOCKER`) либо не трогать (`UNTOUCHED`).

Кошелёк `USER_SHARED` без строки в этой таблице **не соберётся**: `static_assert`
роняет сборку контракта. Это страховка от забывчивости, а не решение задачи:
**какая именно политика у кошелька — вопрос к владельцу, и он решается при
проектировании приложения, а не в момент, когда падает компилятор.** Новое
приложение описывает свои кошельки и их судьбу при выходе вместе с остальной
экономикой; ответ на «пайщик уходит — что с этими деньгами» даёт Положение
ЦПП, и если Положение молчит — спрашиваем.

Правило распространяется на остатки, по которым идёт срок: материалы на
ответственном хранении (`w.edu.hold`) и подотчёт (`w.exp.adv`) держат выход,
потому что кооператив иначе остаётся с чужим имуществом и без стороны
договора.
